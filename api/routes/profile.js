...
const { checkImagesMiddleware, uploadImagesToS3Middleware } = require("../middleware/upload-images-middleware");
...
router.get("/:target", checkTokenMiddleware, checkProfileCreatedMiddleware, async (req, res) => {
  ...
  pool.query(sql, [profileId], async (err, result) => {
    ...
    pool.query(sql3, [req.params.target, result.rows[0].latitude, result.rows[0].longitude], async (err3, result3) => {
      ...
      return res.json({
        "type": type, "result": result3.rows[0],
        "me": {
...
router.post("/me", checkTokenMiddleware, (req, res) => {
  ...
  const createProfileQuery = "INSERT INTO userprofile (first_name, last_name, birth, genre, preference, biograpy, tags, latitude, longitude) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)";
  const arg = [
    req.body.first_name,
    req.body.last_name,
    req.body.birth,
  ...
  pool.query(createProfileQuery, arg, (err, _) => {
    ...
    const getMaxIdQuery = "SELECT MAX(id) AS id FROM userprofile";
    pool.query(getMaxIdQuery, [], (err, result) => {
      if (err) {
        logger.error("error while retrieving max id");
        return res.status(400).json({ message: err.message })
      }
      idMax = result.rows[0].id

      const updateUserLoginIdQuery = "UPDATE userlogin SET id_user_profile = $1 WHERE id = $2";
      pool.query(updateUserLoginIdQuery, [idMax, res.locals.id_user], (err, _) => {
        if (err) {
          logger.error("error updating login");
          return res.status(400).json({ message: err.message })
        }
        logger.info(`created profile with id ${idMax}`);
